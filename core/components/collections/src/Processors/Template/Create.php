...
class Create extends CreateProcessor
{
    ...
    public function validateTemplates($templates)
    {
        ...
        $c->select($this->modx->getSelectColumns(modTemplate::class, 'ResourceTemplate', '', ['templatename']));
        ...
        if ($existingTemplatesCount > 0) {
            ...
            return $this->modx->lexicon('collections.err.template_resource_template_aiu_' . $type, ['templates' => implode(', ', $existingTemplates)]);
        }
    ...
    public function handleSelectionSwitchForGlobalView()
    {
        ...
        $templatesQuery = $this->modx->newQuery(CollectionResourceTemplate::class);
        $templatesQuery->leftJoin(CollectionTemplate::class, 'CollectionTemplate');

        $templatesQuery->where([
            'CollectionTemplate.selection' => 0,
        ]);

        $templatesQuery->select($this->modx->getSelectColumns(CollectionResourceTemplate::class, 'CollectionResourceTemplate', '', ['resource_template']));
        ...
        $resources = $this->modx->getIterator(modResource::class, $resourcesQuery);
    ...
    public function checkResourcesForChildren(array $resources)
    {
        ...
        if (!empty($withChildren)) {
            ...
            return $this->modx->lexicon('collection.err.selection_resource' . $type . '_children', ['resources' => implode('<br />', $withChildren)]);
        }
    ...
    {
        $column = $this->modx->newObject(CollectionTemplateColumn::class);
        $column->set('name', 'id');
        $column->set('label', 'id');
